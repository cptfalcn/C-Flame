...
int main(int argc, char* argv[])
{
	...
	// note: The 2d view uses row major layout.  Most matrix format uses column major layout.
	{
		...
		//Hard code conditions for the NButane experiment.
		data[0]         = 1200.0;
		...
		//PrintNBut(StateData, NumScalarPoints);	
		...
		problem2.pb._p=10*problem2.pb._p; //10 atm
	
		//===================
		//Create Cantera
		//===================
		//Set a solution per the gri standard
		std :: cout << BAR << "Setting Cantera" << BAR << std :: endl;
		auto sol = Cantera::newSolution("nbutane/nbutane.yaml", "gas");
		Set_ThermTransData(&problem2, State, sol);
		std :: cout << BAR << "Cantera is set" << BAR << std :: endl;
		...
		while(StepCount<Steps)//while(TNext<FinalTime)
        {
			...
			//Check heating
			problem2.CheckHeatingFinal(State, TNow, 4e-5);
...
void PrintNBut(realtype * data, int NumScalarPoints)
{
	for(int i = 0; i < NumScalarPoints; i++)
	{
		std :: cout << "Temp=" << data[i] << "\t\t C4H10=" << data[58*NumScalarPoints + i];
		std :: cout <<"\t\t O2=" << data[11*NumScalarPoints + i];	
		std :: cout << std :: endl;
		//data[0]         = 1200.0;
		//data[11]        = 2.173895119224689421e-01;  //O2
		//data[58]        = 6.075041856298054460e-02;  //C4H10
...
// |__||__||____||__||__|  |__|
//==================================
//Bugged, this uses the old table method which will alter the heating regime accuracy
//==================================
...
int Set_ThermTransData(myPb2* pbPtr, N_Vector State, std::shared_ptr<Cantera::Solution> sol)
{
	...
	//std :: cout << "Setting Pointers and cloning" << std :: endl;
	N_Vector yTemp 		= N_VNew_Serial(pbPtr->num_equations);
	//std :: cout << pbPtr->num_equations << std :: endl;
