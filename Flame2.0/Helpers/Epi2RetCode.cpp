...
Epi2RetCode::Epi2RetCode(CVRhsFn f, CVSpilsJacTimesVecFn jtv, void *userData, int maxKrylovIters, N_Vector tmpl, const int vecLength) : NEQ(vecLength)
{
    this->f = f;
    this->jtv = jtv;
    ...
    krylov = new KiopsRetCode(MaxPhiOrder1 + 1, maxKrylovIters, tmpl, NEQ);
...
Epi2RetCode::Epi2RetCode(CVRhsFn f, void *userData, int maxKrylovIters, N_Vector tmpl, const int vecLength) : NEQ(vecLength)
{
    this->f = f;
    this->userData = userData;
    this->jtv = nullptr;
...
IntegratorStats *Epi2RetCode::Integrate(realtype h, realtype t0, realtype tFinal, const int numBands, N_Vector y, realtype krylovTol, int basisSizes[])
{
    if( h < ZERO )
    {
    ...
    for( auto &k : timeSteps )
    {
        ...
        krylov->ComputeKry(2, stage1InputVecs, timePts, 1, stage1OutputVecs, &jtimesv, h, krylovTol, basisSizes[0], &integratorStats->krylovStats[0]);
