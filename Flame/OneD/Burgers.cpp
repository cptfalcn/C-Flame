...
int main(int argc, char *argv[])
{
   ...
   //Initialize States
	...
	realtype ModStepSize = TestStep*1e1;		//StepSize*10;
    	const realtype KrylovTol = RCONST(1.0e-10);	//wtf was it set 1e-17?
	...
	std :: cout << "Completed EpiP2 integration\n";
        auto StopEPIP2 = std::chrono::high_resolution_clock::now();
        auto Pass5 = std::chrono::duration_cast<std::chrono::nanoseconds>(StopEPIP2-StartEPIP2);
	...
	*/
	...
	PrintErrStepTime2File(EPI3State->u, RefState->u, EPI3State->userData->scratch1,
                                Pass4.count()/1e9, ModStepSize, "BurgersDebug.txt", "EPI3");
	...
	PrintErrStepTime2Console(EPIP2State->u, RefState->u, EPIP2State->userData->scratch1,
                                Pass5.count()/1e9, ModStepSize, "EPIP2");
