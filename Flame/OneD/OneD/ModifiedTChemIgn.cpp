...
main(int argc, char* argv[])
{
  ...
  opts.set_option<bool>("use_prefixPath", "If true, input file are at the prefix path", &use_prefixPath);
  ...
  opts.set_option<int>("time-iterations-per-interval",
	"Number of time iterations per interval to store qoi",&num_time_iterations_per_interval);
  opts.set_option<int>("max-time-iterations", "Maximum number of time iterations",
	&max_num_time_iterations);
  opts.set_option<int>("max-newton-iterations","Maximum number of newton iterations",
	&max_num_newton_iterations);
  opts.set_option<int>("output_frequency", "save data at this iterations", &output_frequency);
  ...
  opts.set_option<real_type>("T_threshold", "Temp threshold in ignition delay time", &T_threshold);
  opts.set_option<bool>("OnlyComputeIgnDelayTime", "If true, simulation ends when Temp = T_threshold ",
  ...
  if (r_parse)
    ...
    // if ones wants to all the input files in one directory, and do not want give all names
    if ( use_prefixPath ){
      chemFile      = prefixPath + "chem.inp";
      thermFile     = prefixPath + "therm.dat";
      inputFile     = prefixPath + "sample.dat";
      //printf("Using the prefix path %s \n",prefixPath.c_str() );
    }
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    const TChem::KineticModelConstData<TChem::exec_space> kmcd = kmd.createConstData<TChem::exec_space>();

    const ordinal_type stateVecDim = TChem::Impl::getStateVectorSize(kmcd.nSpec);

    printf("Number of Species: %d \t", kmcd.nSpec);
    ...
    FILE* fout = fopen("DataModifiedIgn.dat", "w");
    std::ofstream myfile("DataTest.txt", std::ios_base::app);

    /// input from a file; this is not necessary as the input is created by other applications.
    ...
    //Defines how to write the state
    auto writeState = [](const ordinal_type iter,
      ...
      for (size_t sp = 0; sp < _state_at_i.extent(0); sp++) {
        fprintf(fout, "%d \t %15.15e \t  %15.15e \t ", iter, _t(sp), _dt(sp));
        //
        for (ordinal_type k = 0, kend = _state_at_i.extent(1); k < kend; ++k)
          fprintf(fout, "%15.15e \t", _state_at_i(sp, k));
	...
	//Defines how to print the state
    ...
    timer.reset();
    {
	  ...
	  //Initial print banner at top of outputfile
          ...
          for (ordinal_type k = 0; k < kmcd.nSpec; k++)//Print Mass Fraction
        ...
        for (; iter < max_num_time_iterations && tsum <= tend; ++iter) {
          ...
          /// Ignition delay time Temperature threshold, QOI Not used, commented out
	  /*
          Kokkos::parallel_for(
            Kokkos::RangePolicy<TChem::exec_space>(0, nBatch),
            KOKKOS_LAMBDA(const ordinal_type& i) {
	...
	/**/
	  ...
	  //Major Print Block
          if (iter % output_frequency == 0 && output_frequency > 0) {
            const ordinal_type indx = iter / output_frequency;
		//Removed because it clogs console
            //printf("save at iteration %d indx %d\n", iter, indx);
            // time, sample, state
            // save time and dt
