...
using std::vector; 
using namespace EpiRK4SC_KIOPSNamespace;
...
EpiRK4SC_KIOPS::EpiRK4SC_KIOPS(CVRhsFn f, CVSpilsJacTimesVecFn jtv, void *userData, int maxKrylovIters, N_Vector tmpl, const int vecLength) : NEQ(vecLength)
{
    this->f = f;
    this->jtv = jtv;
    ...
    krylov2 = new Kiops(MaxPhiOrder2 + 1, maxKrylovIters, tmpl, NEQ);
...
EpiRK4SC_KIOPS::EpiRK4SC_KIOPS(CVRhsFn f, void *userData, int maxKrylovIters, N_Vector tmpl, const int vecLength) : NEQ(vecLength)
{
    this->f = f;
    this->userData = userData;
    this->jtv = nullptr;
...
EpiRK4SC_KIOPS::EpiRK4SC_KIOPS(CVRhsFn f, EPICNumJacDelta delta, void *userData, int maxKrylovIters, N_Vector tmpl, const int vecLength) : NEQ(vecLength)
{
    this->f = f;
    this->userData = userData;
    this->jtv = nullptr;
...
IntegratorStats *EpiRK4SC_KIOPS::Integrate(realtype h, realtype t0, realtype tFinal, const int numBands, N_Vector y, realtype krylovTol, int basisSizes[])
{
    if( h < ZERO )
    {
    ...
    for( auto &k : timeSteps )
    {
        ...
        krylov->Compute(Stage1NumInputVecs, stage1InputVecs, g1Times, g1NumTimes, stage1OutputVecs, &jtimesv, h, krylovTol, basisSizes[0], &integratorStats->krylovStats[0]);
        ...
        N_VLinearSum(1.0, y, 1.0, r1, r1); // r1 = y + r1 = y + phi( ) * hfy
        N_VLinearSum(1.0, y, 1.0, r2, r2); // r2 = y + r1 = y + phi( ) * hfy
        ...
        N_Vector stage3OutputVecs[] = {scratchVec3}; 
        krylov2->Compute(Stage3NumInputVecs, stage3InputVecs, g3Times, g3NumTimes, stage3OutputVecs, &jtimesv, h, krylovTol, basisSizes[1], &integratorStats->krylovStats[1]);  
